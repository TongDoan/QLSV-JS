...
namespace QLSV.Web.Tests
{
    public abstract class QLSVWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static QLSVWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(QLSVWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<QLSVDbContext> action)
        {
            using (var context = IocManager.Resolve<QLSVDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<QLSVDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<QLSVDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<QLSVDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<QLSVDbContext, Task<T>> func)
        {
            T result;
